...
function loadData(page) {
    $.ajax({
        type: "post",
        url: localStorage.getItem("ajaxUrl") + '/coneBarrel/index.do',
        data: {'page': page},
        async: false,
...
function parseResult(objectInfo) {
    ...
    if (objectInfoList.length === 0) {
        ...
        for (var i = 0; i < objectInfoList.length; i++) {
            ...
            obj += '<td class="table-td-content">' + objectInfoList[i].coneBucketNum + '</td>';
            obj += '<td class="table-td-content">' + objectInfoList[i].coneBucketType + '</td>';
            obj += '<td class="table-td-content">' + objectInfoList[i].coneBucketStatus + '</td>';
            obj += '<td class="table-td-content">' + objectInfoList[i].roadName + '</td>';
            obj += '<td class="table-td-content">' + objectInfoList[i].startTime + '</td>';
            obj += '<td class="table-td-content">' + objectInfoList[i].endTime + '</td>';
            obj += '</tr>';
        }
    }
