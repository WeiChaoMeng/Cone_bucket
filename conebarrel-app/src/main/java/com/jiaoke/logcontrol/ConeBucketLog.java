...
public class ConeBucketLog {
    ...
    public void addGoogleConeBucketLog(){
        ...
        for (int i = 0; i < online.size();i++){
            ...
            String coneBucketNum = online.getJSONObject(i).getString("id");
            ...
            Map<String, String> coneBucket = syetemLogInf.getConeBucketLastTimeBydeviceId(coneBucketNum);

            if (coneBucket == null){

                syetemLogInf.addConeBucketTime(coneBucketNum,"1","道路维护",roadName,lasttime,"");

            }else {
                String start_time = coneBucket.get("start_time");
                String dbSubStartTime = start_time.substring(0,10);
                String portSubStartTime = lasttime.substring(0,10);
        ...
        for (int i =0;i < offOnline.size();i++){
            String coneBucketNum = offOnline.getJSONObject(i).getString("id");
            String lasttime = offOnline.getJSONObject(i).getString("lasttime");

            Map<String, String> dbLogData = syetemLogInf.getConeBucketLastTimeBydeviceId(coneBucketNum);
        ...
        if (dataArray.size() != 0){
            for (int i =0;i < dataArray.size();i++){
               ...
               String startDataTime = String.valueOf(startDataTimeTem) + "000";
                ...
                if (res != null){
                    if (HttpClientUtil.timeStamp2Date(startDataTime).equals(res.get("start_time")) &&
                            HttpClientUtil.timeStamp2Date(endDataTime).equals(res.get("end_time"))){
                        continue;
                    }else {
                        //添加锥桶信息
