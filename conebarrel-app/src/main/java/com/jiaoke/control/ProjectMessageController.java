...
//    @RequiresPermissions("projectEntry")
    ...
    public String addEngineering(Model model) {
        ...
        List<ProjectType> projectTypeList = projectTypeService.selectAll();
        //查询所有锥桶
        List<ConeBarrelManage> coneBarrelManageList = coneBarrelManageService.getAll();

        model.addAttribute("coneBucketTypeList", coneBucketTypeList);
        model.addAttribute("projectTypeList", projectTypeList);
        model.addAttribute("coneBarrelManageList", coneBarrelManageList);
        return "project/add";
    }
...
//    @RequiresPermissions("projectEntry")
     ...
     * @return str
     */
    public String getOnLineConeBarrelManage(){
        return "";
    }
    ...
    public String details(Integer id) {
        ...
        return JsonHelper.toJSONString(projectMessage);
    }
...
//    @RequiresPermissions("projectEntry")
    ...
    public String toEdit(Integer id, Model model) {
        ...
        ProjectMessage projectMessage = projectMessageService.selectById(id);
        //工程锥桶信息
        List<ConeBarrelManage> barrelManageList = projectMessage.getConeBarrelManages();
        String coneBucketNum = "";
        for (int i = 0; i < barrelManageList.size(); i++) {
            coneBucketNum += barrelManageList.get(i).getDeviceId();
            if (barrelManageList.size() - 1 != i) {
                coneBucketNum += ",";
            }
        }
        //工程经纬度
