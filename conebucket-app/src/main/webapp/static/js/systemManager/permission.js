...
function loadData(page) {
    $.ajax({
        type: "post",
        url: localStorage.getItem("ajaxUrl") + '/permission/index.do',
        data: {'page': page},
        async: false,
...
function parseResult(objectInfo) {
    ...
    if (objectInfoList.length === 0) {
        obj += '<tr>';
        obj += '<td colspan="3" style="text-align: center;">' + '暂无数据' + '</td>';
        obj += '</tr>';
    } else {
...
function search(page) {
    ...
    $.ajax({
        type: "post",
        url: localStorage.getItem("ajaxUrl") + '/permission/search.do',
